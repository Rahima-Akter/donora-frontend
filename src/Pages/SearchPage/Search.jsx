    ...
    <div className="min-h-screen pb-12">
      ...
      <div className="md:mt-[80px] my-[65px] w-11/12 mx-auto">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5 h-full">
          {searches.length === 0 ? (
            <p className="font-bold drop-shadow-lg col-span-5 uppercase text-Red text-xl mb-4 text-center mt-12">
              Search for donation....
            </p>
          ) : (
            searches.map((search) => (
              <div
                key={search._id}
                className="flex flex-col max-w-sm overflow-hidden bg-red-50 rounded-lg shadow-lg dark:bg-gray-800 border border-red-100 dark:border-none"
              >
                <div className="p-4 md:py-4 md:px-5 w-full flex flex-col flex-grow">
                  <h1 className="text-xl font-bold text-Red">
                    {search.recipientName} ({search.bloodGroup})
                  </h1>
                  ...
                  <div className="flex items-center mt-auto">
                    <h1 className="text-xs font-bold text-Red md:text-sm">
                      {search.status}....
                    </h1>
                    <button
                      onClick={() =>
